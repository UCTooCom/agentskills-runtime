...
AgentSkills Runtime 已实现对《人工智能 智能体互联》国家标准（GB/Z 185.1~185.7-2026）的支持，采用**双模式分层架构**：

- **本地模式（Local Mode）**：面向同一系统内 Agent 间协作场景，复用现有 uctoo_user + RBAC 体系进行身份管理，复用 agent_messages + agent_tasks 进行消息传递，扩展国标数据模型（ACS 描述格式、交互消息/任务/会话结构）。系统默认运行在本地模式。
- **互联模式（Interconnection Mode）**：面向需要与外部系统智能体互联的场景，在本地模式基础上完整实现 GB/Z 185.2~185.7 能力，对接 ACPs 注册服务、CA 服务、发现服务、MQ 服务，叠加 AIC 身份码、CAI 凭证、mTLS 认证等分布式能力。互联模式是本地模式的超集。
...
| `identity_status` | varchar(20) | AIP身份状态：none/active/locked/revoked |
...
AGENTS.md 的 YAML frontmatter 已扩展支持 AIP 字段：

```yaml
---
name: MainAgent
agent_type: main
description: 主 Agent 描述
identity_status: none          # AIP身份状态
discoverable: true             # 是否允许被发现
capabilities: []               # 辅助功能描述(ACS)
default_input_types: []        # 默认输入类型
default_output_types: []       # 默认输出类型
---
```

#### MQ 消息分发适配器

互联模式下通过 AcpsMqAdapter 实现 GB/Z 185.6 群组交互消息的可靠分发，基于 activemq4cj（仓颉 ActiveMQ SDK，JMS 2.0 规范实现）封装，支持：
- 点对点和发布/订阅双模式消息通道
- 持久订阅与事务消息
- Failover 自动重连
- TLS 加密连接
...
> 详细设计文档参见 `.codeartsdoer/specs/aip-implementation/` 和 `.codeartsdoer/specs/aip-mq-adapter/`。
...
### 多语言 SDK 支持
- **JavaScript/TypeScript SDK**：完整的 Node.js 和浏览器环境支持，用于 Web 应用和前端集成
- **Python SDK**：集成流行的 Python AI 和数据科学库，适用于数据分析和 AI 模型集成
- **Java SDK**：企业级应用和 Android 平台支持，适用于大型企业系统
- **Go SDK**：高性能并发处理和云原生应用支持，适用于微服务架构
- **Rust SDK**：系统级性能和内存安全保证，适用于对性能要求高的场景
- **C# SDK**：.NET 生态系统和 Windows 平台集成，适用于企业内部系统

### SDK 核心价值
- **传统系统 + AI**：为现有数字系统快速集成 AI 能力，无需重构现有架构
- **技能市场对接**：通过 SDK 轻松访问技能市场，实现能力无限扩展
- **标准化集成**：提供统一的 API 接口，简化跨语言集成复杂度
- **开箱即用**：内置运行时安装和管理功能，无需手动配置运行环境
- **跨平台支持**：同一套 API 适配不同技术栈，降低学习成本
...
1. **克隆 UCToo 项目**
   ```bash
   git clone https://atomgit.com/UCToo/web-admin.git
   ```

2. **运行安装助手**
   - 双击运行 `web-admin/start-installer.bat`（Windows系统）
   - 或在命令行中执行：
     ```bash
     cd web-admin
     start-installer.bat
