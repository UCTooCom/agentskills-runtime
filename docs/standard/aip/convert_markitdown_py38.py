# -*- coding: utf-8 -*-
"""
使用 MarkItDown 源码转换 PDF（适配 Python 3.8）
直接复制并适配关键模块，避免 Python 版本不兼容
"""
...
class DocumentConverterResult:
    ...
    @text_content.setter
    def text_content(self, markdown):
        self.markdown = markdown
...
def _extract_form_content_from_words(page):
    """从 PDF 页面提取表单/表格内容（MarkItDown 核心算法）"""
...
def convert_pdf_with_markitdown(pdf_path):
    """使用 MarkItDown 风格的 PDF 转换"""
    import pdfplumber
        ...
        for page_idx, page in enumerate(pdf.pages):
            ...
            if page_content is not None:
                ...
                if page_content.strip():
                    page_starts.append(len(markdown_chunks))
...
def pdf_to_markdown(pdf_path, output_dir):
    ...
    # 使用 MarkItDown 风格转换文本
    markdown = convert_pdf_with_markitdown(pdf_path)
