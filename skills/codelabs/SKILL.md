...
description: >
  ...
  provided by the codelabs plugin. Supports create, update, soft-delete,
  restore, paged list with Prisma-style filter/sort. Triggers on:
  {{tableName}} data management requests, codelabs plugin capability discovery.
license: MIT
plugin: codelabs
service: skill_codelabs.CodelabsService
route: /api/v1/{{dbName}}/{{tableName}}
---

# Codelabs CRUD Skill

本技能由 codelabs 插件三维一体提供（Service + Skill + Route），
...
1. **服务调用（确定性通道）**：宿主经 `ServiceRegistry.getService<CodelabsService>()` 类型安全轨获取服务。
2. **HTTP 调用（插件路由通道）**：
