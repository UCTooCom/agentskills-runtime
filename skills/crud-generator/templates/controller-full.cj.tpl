...
import magic.app.services.{DATABASE_NAME}.{TABLE_NAME_PASCAL}Service
...
public class {TABLE_NAME_PASCAL}Controller {
    private var service: {TABLE_NAME_PASCAL}Service
    
    public init(service: {TABLE_NAME_PASCAL}Service) {
                ...
                if (let Some(idsVal) <- ids) {
                    ...
                    if (let Some(idsStr) <- idsStrOpt) {
                        ...
                        if (isRestore) {
                            ...
                            let entities = ArrayList<{TABLE_NAME_PASCAL}PO>()
    ...
    public func getSingle(req: HttpRequest, res: HttpResponse): Unit {
        try {
            LogUtils.info("{TABLE_NAME_PASCAL}Controller", "=== {TABLE_NAME_PASCAL}Controller.getSingle() called ===")
            LogUtils.info("{TABLE_NAME_PASCAL}Controller", "=== Request path: ${req.uri.path} ===")
            let id = req.pathParam("id")
            
            if (let Some(idVal) <- id) {
                LogUtils.info("{TABLE_NAME_PASCAL}Controller", "=== Got id value: ${idVal} ===")
                let result = service.getById(idVal)
                let reasonStr = result.reason ?? "none"
                LogUtils.info("{TABLE_NAME_PASCAL}Controller", "=== service.getById result - success: ${result.success}, reason: ${reasonStr} ===")
                
                if (result.success) {
                    if (let Some(data) <- result.data) {
                        LogUtils.info("{TABLE_NAME_PASCAL}Controller", "=== {TABLE_NAME_PASCAL} found, returning data ===")
                        // 与backend保持一致，直接返回实体对象，不包装在data中
                        res.status(200).json(data.toJson())
                    } else {
                        LogUtils.info("{TABLE_NAME_PASCAL}Controller", "=== result.data is None ===")
                        res.status(404).json("{\"errno\":\"40401\",\"errmsg\":\"未找到该{TABLE_NAME_CAMEL}或您没有权限访问\"}")
                    }
                } else {
                    LogUtils.info("{TABLE_NAME_PASCAL}Controller", "=== result.success is false ===")
                    res.status(404).json("{\"errno\":\"40401\",\"errmsg\":\"未找到该{TABLE_NAME_CAMEL}\"}")
                }
            } else {
                LogUtils.info("{TABLE_NAME_PASCAL}Controller", "=== id pathParam is None ===")
                res.status(400).json("{\"errno\":\"40002\",\"errmsg\":\"缺少ID参数\"}")
            }
        } catch (e: Exception) {
            LogUtils.error("{TABLE_NAME_PASCAL}Controller", "=== {TABLE_NAME_PASCAL}Controller.getSingle() exception: ${e.message} ===")
            res.status(500).json("{\"errno\":\"50000\",\"errmsg\":\"${e.message}\"}")
        }
    }
    ...
    public func getManyWithPathParams(req: HttpRequest, res: HttpResponse): Unit {
        try {
            // 按照 backend 规范：/api/{DATABASE_NAME}/{TABLE_NAME}/:limit/:page
            ...
            res.status(200).json("{\"currentPage\":${pageNum},\"totalCount\":${total},\"totalPage\":${totalPage},\"{TABLE_NAME}s\":[${entitiesJson}]}")
        } catch (e: Exception) {
    ...
    public func getManyWithSkip(req: HttpRequest, res: HttpResponse): Unit {
        try {
            // 按照 backend 规范：/api/{DATABASE_NAME}/{TABLE_NAME}/:limit/:page/:skip
            // 页码从1开始，1表示第一页（符合业界最佳实践）
            let limitParam = req.pathParam("limit")
            let pageParam = req.pathParam("page")
            ...
            res.status(200).json("{\"currentPage\":${pageNum},\"totalCount\":${total},\"totalPage\":${totalPage},\"skip\":${skipNum},\"{TABLE_NAME}s\":[${entitiesJson}]}")
        } catch (e: Exception) {
            res.status(500).json(APIError("50000", e.message).toJson())
        }
    }
