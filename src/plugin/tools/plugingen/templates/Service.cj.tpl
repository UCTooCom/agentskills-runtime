 ...
 * 插件轨实现（plugingen 生成）：经 Controller 接线点暴露，注册名
 * "skill_{{name}}.{{className}}Service"。
 */
public class {{className}}Service {
    ...
    /**
     * 更新{{tableName}}（无权限检查，字段合并：仅更新非默认值字段）
     */
    ...
    /**
     * 获取{{tableName}}列表（基础分页，无过滤排序，用于导出）
     */
    ...
    /**
     * 获取{{tableName}}列表（分页，Prisma 风格 filter/sort）
     *
     * 支持的查询参数：
    ...
    /**
     * 获取{{tableName}}列表（分页，带 skip 跳过参数，Prisma 风格 filter/sort）
     */
    ...
    public func getListWithSkipWithPermission(
        ...
        try {
            ...
            let (permFilter, _) = PermissionUtils.appendPermissionFilter(userId, "{{tableName}}")
